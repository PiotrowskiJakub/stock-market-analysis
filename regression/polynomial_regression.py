...
predictions = regressor.predict(poly_reg.fit_transform(X_test))
score = regressor.score(poly_reg.fit_transform(X_test), y_test)


# Only for one dimensional X matrix
def show_plot(X, y, regressor, poly_reg):
    X_grid = np.arange(min(X), max(X), 0.1)
    X_grid = X_grid.reshape((len(X_grid), 1))
    plt.scatter(X, y, color = 'red')
    plt.plot(X_grid, regressor.predict(poly_reg.fit_transform(X_grid)), color = 'blue')
    plt.title('Stock polynomial regression')
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.show()
    
# Visualising the Polynomial Regression train results
